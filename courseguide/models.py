...
class Course(models.Model):
    ...
    def __str__(self):
        return (
            f"The name of the course is {self.course_name} "
            f"and the slug is {self.slug}"
        )
...
class HoleGuide(models.Model):
    ...
    stroke_index = models.PositiveSmallIntegerField(
        help_text=(
            "1 = hardest hole, "
            "18 = easiest"
        )
    ...
    guide = models.TextField(
        help_text="Strategic tips or description for playing the hole."
    )
    ...
    class Meta:
        ...
        unique_together = (
            'course',
            'hole_number'
        )  # ensures each hole number is unique per course
